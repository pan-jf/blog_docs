...
我们知道，一切的软件都是跑在操作系统上，真正用来干活 (计算) 的是 CPU。
早期的操作系统每个程序就是一个进程，直到一个程序运行完，才能进行下一个进程。
这就是 “单进程时代”，一切的程序只能串行发生。


![image.png](../../../assets/content/golang/202206/单进程时代.jpg)
...
### (2) 多进程 / 线程时代有了调度器需求
![image.png](../../../assets/content/golang/202206/多进程操作系统.jpg)
...
![image.png](../../../assets/content/golang/202206/CPU切换成本.jpg)
...
好了，然后工程师们就发现，其实一个线程分为 `"内核态"线程`和`"用户态"线程`。

一个 `"用户态"线程`必须要绑定一个 `"内核态"线程`，但是 CPU 并不知道有`"用户态"线程`的存在，它只知道它运行的是一个`"内核态"线程`(Linux 的 PCB 进程控制块)。

![image.png](../../../assets/content/golang/202206/线程空间.jpg)

这样，我们再去细化去分类一下，`"内核态"线程`依然叫 `“线程 (thread)”`，`"用户态"线程`叫 `“协程 (co-routine)”`.

![image.png](../../../assets/content/golang/202206/线程空间2.jpg)
...
![image.png](../../../assets/content/golang/202206/协程线程n_1.jpg)
...
![image.png](../../../assets/content/golang/202206/协程线程1_1.jpg)
...
![image.png](../../../assets/content/golang/202206/协程线程n_m.jpg)
...
![image.png](../../../assets/content/golang/202206/GM定义.jpg)

下面我们来看看被废弃的 golang 调度器是如何实现的？
![image.png](../../../assets/content/golang/202206/旧版调度器.jpg)

M 想要执行、放回 G 都必须访问`全局 G 队列`，并且 M 有多个，即多线程访问同一资源需要加锁进行保证互斥 / 同步，所以`全局 G 队列`是有`互斥锁`进行保护的。
...
![image.png](../../../assets/content/golang/202206/GMP定义.jpg)

Processor，它包含了运行 goroutine 的资源，如果线程想运行 goroutine，必须先获取 P，P 中还包含了可运行的 G 队列。

### (1) GMP 模型
在 Go 中，线程是运行 goroutine 的实体，调度器的功能是把可运行的 goroutine 分配到工作线程上。
![image.png](../../../assets/content/golang/202206/GMP调度.jpg)
...
### (3) go func () 调度流程
![image.png](../../../assets/content/golang/202206/go_func调度过程.jpg)
...
### (4) 调度器的生命周期
![image.png](../../../assets/content/golang/202206/调度器生命周期.png)
#### 特殊的 M0 和 G0
...
![image.png](../../../assets/content/golang/202206/可视化调度1.jpg)

![image.png](../../../assets/content/golang/202206/可视化调度2.jpg)
...
![image.png](../../../assets/content/golang/202206/可视化调度3.jpg)
...
![image.png](../../../assets/content/golang/202206/可视化调度4.jpg)
一共有两个 M 在程序中，一个是特殊的 M0，用于初始化使用，这个我们不必讨论。

**P 信息**
![image.png](../../../assets/content/golang/202206/可视化调度5.jpg)
G1 中调用了 main.main，创建了 trace goroutine g18。G1 运行在 P1 上，G18 运行在 P0 上。

这里有两个 P，我们知道，一个 P 必须绑定一个 M 才能调度 G。

我们在来看看上面的 M 信息。
![image.png](../../../assets/content/golang/202206/可视化调度6.jpg)
我们会发现，确实 G18 在 P0 上被运行的时候，确实在 Threads 行多了一个 M 的数据，点击查看如下：
![image.png](../../../assets/content/golang/202206/可视化调度7.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景1.jpg)

### (2) 场景 2

G1 运行完成后 (函数：goexit)，M 上运行的 goroutine 切换为 G0，G0 负责调度时协程的切换（函数：schedule）。从 P 的本地队列取 G2，从 G0 切换到 G2，并开始运行 G2 (函数：execute)。实现了线程 M1 的复用。
![image.png](../../../assets/content/golang/202206/go调度场景2.jpg)

### (3) 场景 3

假设每个 P 的本地队列只能存 3 个 G。G2 要创建了 6 个 G，前 3 个 G（G3, G4, G5）已经加入 p1 的本地队列，p1 本地队列满了。
![image.png](../../../assets/content/golang/202206/go调度场景3.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景4.jpg)
这些 G 被转移到全局队列时，会被打乱顺序。所以 G3,G4,G7 被转移到全局队列。

### (5) 场景 5

G2 创建 G8 时，P1 的本地队列未满，所以 G8 会被加入到 P1 的本地队列。
![image.png](../../../assets/content/golang/202206/go调度场景5.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景6.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景7.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景8.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景9.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景10.jpg)
...
![image.png](../../../assets/content/golang/202206/go调度场景11.jpg)
